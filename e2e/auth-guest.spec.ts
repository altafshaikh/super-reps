...
test.describe('auth (guest / validation)', () => {
  test('unauthenticated root sends user to login', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByTestId('login-identifier')).toBeVisible({ timeout: 60_000 });
  ...
  test('login: empty fields shows form error', async ({ page }) => {
    ...
    await expect(page.getByText('Please enter your email or username and password.')).toBeVisible();
  });

  test('login: invalid email format', async ({ page }) => {
    await page.goto('/login');
    await page.getByTestId('login-identifier').fill('not-an-email@');
  ...
  test('login: wrong password for unknown user', async ({ page }) => {
    ...
    await page.goto('/login');
    await page.getByTestId('login-identifier').fill('e2e-does-not-exist@example.com');
    await page.getByTestId('login-password').fill('WrongPass1');
    await page.getByTestId('login-submit').click();
    await expect(
      page.getByText(/invalid email, username, or password|network error|could not sign in/i),
